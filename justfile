...
# Compile Tailwind v4 CSS (devShell only) → committed theme asset.
# Hugo serves the output verbatim; the Nix image build needs no Node.
tw:
    cd site && npx @tailwindcss/cli -i src/input.css -o themes/efi-theme/assets/css/main.css --minify

# Watch Tailwind source and rebuild on change. Run alongside `just serve`.
tw-watch:
    cd site && npx @tailwindcss/cli -i src/input.css -o themes/efi-theme/assets/css/main.css --watch
